...
def load_corte(sheet):
    ...
    required = ['CodEst','NER','Centro','Programa','Modalidad','Turno','TipoArea','Nivel','MI','MA','APR','Rep1','Rep2','Rep3mas','TotalRep']
    missing = [c for c in required if c not in df.columns]
    if missing:
        raise ValueError(f'Hoja {sheet}: faltan columnas tras el mapeo: {missing}. Columnas originales: {list(df.columns)}')
...
# Verificacion rapida de consistencia (se imprime al final del script)
CHECK_I_MA = int(df_i['MA'].sum())
CHECK_II_MA = int(df_ii['MA'].sum())
...
CHECK_ASISDOC_I = asis_rate(asis_doc, MESES_I, 'Total_Docente')
CHECK_ASISDOC_II = asis_rate(asis_doc, MESES_II, 'Total_Docente')
...
# ---------------------------------------------------------------
# 4. CLASES REPROBADAS -> contar por asignatura y por corte
# ---------------------------------------------------------------
def norm_asig(s):
    s = s.strip()
    fixes = {
...
# ---------------------------------------------------------------
# 7. Verificaciones de integridad (fallan ruidosamente si algo esta mal)
# ---------------------------------------------------------------
assert len(final['i_corte']['grado_muni']) > 0, 'grado_muni I vacio'
assert len(final['ii_corte']['grado_muni']) > 0, 'grado_muni II vacio'
assert len(final['i_corte']['modalidad_muni']) > 0, 'modalidad_muni I vacio'
assert len(final['ii_corte']['modalidad_muni']) > 0, 'modalidad_muni II vacio'
assert final['i_corte']['municipio']['AsisDocente'] is not None, 'AsisDocente I es None'
assert final['ii_corte']['municipio']['AsisDocente'] is not None, 'AsisDocente II es None'
assert final['i_corte']['municipio']['MA'] == CHECK_I_MA, 'Descuadre en MA del I Corte'
assert final['ii_corte']['municipio']['MA'] == CHECK_II_MA, 'Descuadre en MA del II Corte'
assert len(final['i_corte']['ner_list']) == 26, f"Se esperaban 26 NER en I Corte, hay {len(final['i_corte']['ner_list'])}"
assert len(final['ii_corte']['ner_list']) == 26, f"Se esperaban 26 NER en II Corte, hay {len(final['ii_corte']['ner_list'])}"

print('=== VERIFICACION OK ===')
print('Tamano archivo:', len(json.dumps(final)))
print('Municipio I  -> MI:', final['i_corte']['municipio']['MI'], ' MA:', final['i_corte']['municipio']['MA'],
      ' Permanencia:', final['i_corte']['municipio']['Permanencia'], ' AsisDocente:', final['i_corte']['municipio']['AsisDocente'])
print('Municipio II -> MI:', final['ii_corte']['municipio']['MI'], ' MA:', final['ii_corte']['municipio']['MA'],
      ' Permanencia:', final['ii_corte']['municipio']['Permanencia'], ' AsisDocente:', final['ii_corte']['municipio']['AsisDocente'])
print('grado_muni I:', len(final['i_corte']['grado_muni']), 'filas | modalidad_muni I:', len(final['i_corte']['modalidad_muni']), 'filas')
print('grado_muni II:', len(final['ii_corte']['grado_muni']), 'filas | modalidad_muni II:', len(final['ii_corte']['modalidad_muni']), 'filas')
