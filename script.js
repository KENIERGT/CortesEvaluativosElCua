...
function ensureChartJs(timeoutMs = 6000){
...
/* Crea un chart protegido: si Chart.js lanza un error con estos datos,
   se muestra un aviso en vez de romper el resto del dashboard. */
function safeChart(key, canvasId, config){
  ...
  if(!window.__chartsOk){
    showChartWarning(holder, '⚠ No se pudo cargar Chart.js (revisa tu conexión a internet). El resto del dashboard funciona con normalidad; recarga la página cuando tengas mejor señal.');
    return;
  }
  try{
    charts[key] = new Chart(canvas, config);
  } catch(e){
    console.error(`Error creando el gráfico "${key}":`, e);
    showChartWarning(holder, '⚠ No se pudo dibujar este gráfico por un problema con los datos. El resto del dashboard sigue funcionando.');
    sectionErrors.push(key);
  }
}

/* Ejecuta una función de render de forma aislada: si falla, se registra
   el error y se continúa con las demás secciones en vez de detener todo. */
function safeRun(name, fn){
  try{
    fn();
  } catch(e){
    console.error(`Error en la sección "${name}":`, e);
    sectionErrors.push(name);
  }
}

function reportSectionErrors(){
  ...
  box.style.cssText = 'display:block;max-width:1200px;margin:0 auto 20px;padding:14px 18px;background:#FBF6EA;border:1px solid #E3D4A8;border-radius:8px;color:#8A6D3B;font-size:13px;';
  box.textContent = `⚠ Algunas secciones no cargaron correctamente (${sectionErrors.join(', ')}). Intenta recargar la página; si el problema persiste, puede ser tu conexión a internet.`;
}

/* ---------------------------------------------------------
   Carga de datos (con cache-busting para evitar versiones viejas
   guardadas por el navegador si la conexión se corta a medias)
--------------------------------------------------------- */
async function initDashboard(){
 try {
  ...
  DATA = await res.json();
  if(!DATA || !DATA.i_corte || !DATA.ii_corte){
    throw new Error('El archivo data.json se cargó pero está incompleto (faltan i_corte o ii_corte). Vuelve a generar el archivo.');
  }
...
function renderAll(){
  ...
  safeRun('encabezado', ()=> renderHero(C, label, periodo));
  safeRun('KPIs', ()=> renderKpis(C, label));
  safeRun('tendencia', ()=> renderTrendChart(C));
  safeRun('programa', ()=> renderProgramaChart(C));
  safeRun('grado', ()=> renderGrado(C));
  safeRun('modalidad', ()=> renderModalidad(C));
  safeRun('asignaturas', ()=> renderAsignaturas());
  safeRun('tabla NER', ()=> renderNerTable(C));
  safeRun('retiros por NER', ()=> renderRetirosNerChart(C));
  safeRun('detalle por NER', ()=> renderNerPickerAndDetail(C));
  safeRun('top general', ()=> renderTopGeneral(C));
  safeRun('rankings', ()=> renderRankings(C));
  safeRun('causas de retiro', ()=> renderRetiros());
...
function renderProgramaChart(C){
  ...
  safeChart('programa', 'chartPrograma', {
    type:'bar',
    data:{
      labels: progLabels,
      datasets:[
...
function renderGrado(C){
  ...
  if(!rows || !rows.length){
    document.getElementById('gradoTableBody').innerHTML = '<tr><td colspan="8" style="text-align:center;color:var(--ink-soft)">Sin datos de grado disponibles.</td></tr>';
    return;
  }

  const h1 = document.getElementById('chartGradoReprob').parentElement;
  h1.innerHTML = '<canvas id="chartGradoReprob"></canvas>';
  safeChart('gradoReprob', 'chartGradoReprob', {
    type:'bar',
    data:{
  ...
  safeChart('gradoRetiros', 'chartGradoRetiros', {
    type:'bar',
    data:{
      labels: rows.map(r=>r.Grado),
...
function renderModalidad(C){
  ...
  if(!rows || !rows.length){
    document.getElementById('modalidadTableBody').innerHTML = '<tr><td colspan="8" style="text-align:center;color:var(--ink-soft)">Sin datos de modalidad disponibles.</td></tr>';
    return;
  }
...
function renderRetirosNerChart(C){
  ...
  safeChart('retirosNer', 'chartRetirosNer', {
    type:'bar',
    data:{
...
function renderRetiros(){
  ...
  safeChart('retiros', 'chartRetiros', {
    type:'bar',
    data:{
